# This module keeps all utility functions for Motif-Z software
...
#   -- n, n choose k
#   -- k, 
#   -- p, NOTE: if p == 0 or 1, which is either background doesnxt have r/p pair or it is 
...
# == Function Summary
#   From produced motifs regular expression format to generate a list of rp ids for presentation
...
#   Return: rp_ids(a list of tuples with postion of residue and its correspoinding iteration, starting from 1st iteration)
def rp_iteration_id(rp_pairs, rexp):
    ...
    replicate_r = list() # Repeated residue list for scanning rexp and decide which residue is not unique
    unique_r = set() # Unique residue set for detect repeated ones
    for rp in rp_pairs:
	if rp[0] not in unique_r:
	    unique_r.add(rp[0])
	else:
	    replicate_r.append(rp[0])

    for rp in rp_pairs:
	for i,r in enumerate(rexp): # index both position and element in rexp string
	    if r==rp[0]:
		if (r not in replicate_r): #only one residue in the rexp to  match
                    rp_ids.append(i)
		elif (i==rp[1] or i-len(rexp)==rp[1]): # check boht positive and negative position
		    rp_ids.append(i) #residue and position both match
		else:
		    print "Error in rp pair pattern match to regex!"
    return rp_ids
...
def format_to_rexp(rp_pairs, MOTIF_LENGTH, MINIMUM_LENGTH):
    ...
    #sort motif pairs based on their positions, from smallest to highest, i.e. negative to positive
...
def categoryPepByLength(pep_set, LIST_KMERS):
    #result_dict = {k : set([]) for k in LIST_KMERS} #Return a result dictionary with length of peptides as key, feature from v2.7
    result_dict = {}
    for k in LIST_KMERS:
        result_dict[k] = set([])
    for pep in pep_set:
