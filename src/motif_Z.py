...
# == Summary
#   Name: Motif-Z for Class I peptide motif
...
def runScript(file_type, peps, data_summary, background_peps, result_file_path, statistics_file_path, debug_file_path, max_length, min_length, zscore_flag,
...
# == Data structure used to store FG and BG peptide sequences: Set
	...
	b_peptide = re.sub("[^ARNDCEQGHILKMFPSTWYV]", "U", b_pep)
        ...
        if len(b_peptide) <=MAX_LENGTH and len(b_peptide) >= MIN_LENGTH:
            num_bg_pep += 1
            background_peps_set.add(b_peptide)
            length = len(b_peptide)
...
#     print("Start recursion")
	...
	p_value = '%.2E' % exp(result_motif[rexp_motif][8])
        score = result_motif[rexp_motif][4]
	rp_iteration_id = motifs_dict[rexp_motif][1]
        result_table_1.append([rexp_motif, rp_iteration_id, total_enrich, match_fg, p_value, score])
    
    # Sort the table based on the score
    sorted_result_table_1 = sorted(result_table_1, key=itemgetter(5), reverse=True)
    # Format score to 2 decimal 
    # Put all motifs in a list with table 1 sequence
    sorted_all_motif = []
    for item in sorted_result_table_1:
	score = '%.2f' % item[5]
	item[5] = score
	sorted_all_motif.append([item[0], item[1]])

    # Build a dictionary to store motif regular expression results and use this dictionary in the webpage output
    result_table_2 = []
    for key in range(7,15):
	if key in statistics_pep_fg.keys():
            k_mer = key
            total_in_fg = data_summary[key]
            unique_in_fg = statistics_pep_fg[key]
            result_table_2.append([k_mer, total_in_fg, unique_in_fg])
	else:
	    result_table_2.append([key, 0, 0])

    sorted_result_table_2 = sorted(result_table_2, key=itemgetter(0))
        
    # Build a dictionary to store motif regular expression results and use this dictionary in the webpage output
    stat_result_dict = {}
    #motif_stat_dict = {k : {} for k in motifs_dict.keys()}
    motif_stat_dict = {}
    for k in motifs_dict.keys():
        motif_stat_dict[k]={}
    ...
    for rexp_motif_str in motifs_dict:
		...
		motif_stat_dict[rexp_motif_str][kmer] = utility.number_matched_pairs(fg_category[kmer], motifs_dict[rexp_motif_str][0])
	    else:
    ...
    #Sort dictionary to a list of list
    sorted_result_table_3 = []
    for motif in sorted_all_motif:
        item = []
	item.append(motif[0])
	item.append(motif[1])
	for i in range(9):
	    item.append(stat_result_dict[motif[0]][i])
	sorted_result_table_3.append(item)
    ...
    results = [sorted_result_table_1, sorted_result_table_2, sorted_result_table_3, debug_file_path]
    return results
